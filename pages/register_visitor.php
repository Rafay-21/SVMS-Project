...
$page_title    = __t('register_visitor');
$page_extra_js = ['webcam.js', 'validation.js'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$host)    $errors[] = 'Host name is required.';
    if (!$purpose) $errors[] = 'Purpose of visit is required.';

    if (empty($errors)) {
        // Check blacklist
        $blacklisted = query_one(
            'SELECT id FROM blacklist WHERE (cnic = ? OR phone = ?) AND is_active = 1 LIMIT 1',
            'ss', [$cnic, $phone]
        );
        if ($blacklisted) {
            $errors[] = 'This visitor is on the blacklist and cannot be registered.';
            flash('error', 'Blacklisted visitor attempted entry. Alert logged.');
        }
    }

    if (empty($errors)) {
        $badge      = generate_badge_number();
        $qr_token   = generate_qr_token();
        $photo_path = '';

        // Save photo
        if ($photo_b64 && strpos($photo_b64, 'data:image') === 0) {
            $img_data = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $photo_b64));
            $dir = UPLOAD_DIR . 'visitor_photos/';
            if (!is_dir($dir)) mkdir($dir, 0750, true);
            $filename = 'photo_' . bin2hex(random_bytes(8)) . '.jpg';
            if ($img_data && file_put_contents($dir . $filename, $img_data)) {
        ...
        $visitor_id = null;
        $stmt = $conn->prepare(
            'INSERT INTO visitors (name, cnic, phone, email, host_name, department, purpose, vehicle_number, items_carried, photo_path, badge_number, qr_token, created_by, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())'
        );
        if ($stmt) {
            $admin_id = (int)$_SESSION['admin_id'];
            $stmt->bind_param('ssssssssssssi', $name, $cnic, $phone, $email, $host, $dept, $purpose, $vehicle, $items, $photo_path, $badge, $qr_token, $admin_id);
            $stmt->execute();
        ...
        if ($visitor_id) {
            log_action('register_visitor', $visitor_id, json_encode(['name' => $name, 'badge' => $badge]));
            flash('success', 'Visitor registered successfully. Badge: ' . $badge);
            header('Location: ' . BASE_URL . 'pages/checkin_checkout.php?visitor_id=' . $visitor_id);
            exit;
        } else {
            $errors[] = 'Failed to save visitor record. Please try again.';
        }
    }
}
...
<div class="container">
  <div class="page-header">
    <div>
      <h1 class="page-title"><i class="bi bi-person-plus-fill" style="color:var(--secondary);"></i> <?= __t('register_visitor') ?></h1>
      <p class="page-subtitle">Register a new visitor and issue a badge.</p>
    </div>
    <a href="<?= BASE_URL ?>pages/visitor_history.php" class="btn btn-secondary"><i class="bi bi-clock-history"></i> View History</a>
  </div>

  <?php foreach ($errors as $err): ?>
    <div class="alert alert-error"><i class="alert-icon bi bi-x-circle-fill"></i><div class="alert-body"><?= e($err) ?></div></div>
  <?php endforeach; ?>
  ...
  <form method="POST" action="" data-validate="true" novalidate>
    ...
    <div class="grid-2">
      ...
      <div>
        <div class="form-section">
          <div class="form-section-title"><i class="bi bi-person-badge"></i> Visitor Information</div>
          <div class="form-group">
            <label for="name">Full Name <span class="required">*</span></label>
            <input type="text" id="name" name="name" class="form-control" value="<?= e($_POST['name'] ?? '') ?>" data-rules="required" required placeholder="e.g. Muhammad Ali">
          </div>
          <div class="form-row">
            <div class="form-group">
              <label for="cnic">CNIC</label>
              <input type="text" id="cnic" name="cnic" class="form-control" value="<?= e($_POST['cnic'] ?? '') ?>" data-format="cnic" placeholder="12345-1234567-1" maxlength="15">
            </div>
            <div class="form-group">
              <label for="phone">Phone</label>
              <input type="tel" id="phone" name="phone" class="form-control" value="<?= e($_POST['phone'] ?? '') ?>" data-format="phone" placeholder="+92 300 1234567">
            </div>
          </div>
          <div class="form-group">
            <label for="email">Email</label>
            <input type="email" id="email" name="email" class="form-control" value="<?= e($_POST['email'] ?? '') ?>" placeholder="visitor@example.com">
          </div>
          <div class="form-group">
            <label for="vehicle">Vehicle Number</label>
            <input type="text" id="vehicle" name="vehicle" class="form-control" value="<?= e($_POST['vehicle'] ?? '') ?>" placeholder="e.g. ABC-123">
          </div>
          <div class="form-group">
            <label for="items">Items Carried</label>
            <textarea id="items" name="items" class="form-control" rows="2" placeholder="Laptop, bag..."><?= e($_POST['items'] ?? '') ?></textarea>
          </div>
        </div>

        <div class="form-section">
          <div class="form-section-title"><i class="bi bi-building"></i> Visit Details</div>
          <div class="form-group">
            <label for="host">Host Name <span class="required">*</span></label>
            <input type="text" id="host" name="host" class="form-control" value="<?= e($_POST['host'] ?? '') ?>" data-rules="required" required placeholder="Person being visited">
          </div>
          <div class="form-group">
            <label for="department">Department</label>
            <input type="text" id="department" name="department" class="form-control" value="<?= e($_POST['department'] ?? '') ?>" placeholder="e.g. HR, Finance">
          </div>
          <div class="form-group">
            <label for="purpose">Purpose of Visit <span class="required">*</span></label>
            <select id="purpose" name="purpose" class="form-control" data-rules="required" required>
              <option value="">Select purpose…</option>
              <option value="Meeting"     <?= ($_POST['purpose'] ?? '') === 'Meeting'     ? 'selected' : '' ?>>Meeting</option>
              <option value="Interview"   <?= ($_POST['purpose'] ?? '') === 'Interview'   ? 'selected' : '' ?>>Interview</option>
              <option value="Delivery"    <?= ($_POST['purpose'] ?? '') === 'Delivery'    ? 'selected' : '' ?>>Delivery</option>
              <option value="Maintenance" <?= ($_POST['purpose'] ?? '') === 'Maintenance' ? 'selected' : '' ?>>Maintenance</option>
              <option value="Personal"    <?= ($_POST['purpose'] ?? '') === 'Personal'    ? 'selected' : '' ?>>Personal</option>
              <option value="Government"  <?= ($_POST['purpose'] ?? '') === 'Government'  ? 'selected' : '' ?>>Government / Official</option>
      ...
      <div>
        <div class="form-section" style="text-align:center;">
          <div class="form-section-title"><i class="bi bi-camera-fill"></i> Visitor Photo</div>
          <div style="position:relative;display:inline-block;margin-bottom:16px;">
            <video id="webcam-video" width="280" height="280" style="border-radius:50%;object-fit:cover;border:4px solid var(--secondary);display:none;" autoplay muted playsinline></video>
            <canvas id="webcam-canvas" width="280" height="280" style="border-radius:50%;object-fit:cover;border:4px solid var(--success);display:none;"></canvas>
            <img src="<?= BASE_URL ?>assets/img/default-avatar.svg" id="photo-placeholder" width="280" height="280" style="border-radius:50%;border:4px dashed var(--border);" alt="Click start camera">
          </div>
          <input type="hidden" name="photo_data" id="webcam-hidden">
          <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
            <button type="button" class="btn btn-secondary" id="start-webcam-btn" onclick="document.getElementById('photo-placeholder').style.display='none';Webcam.init('webcam-video','webcam-canvas','capture-btn','retake-btn','webcam-hidden');">
              <i class="bi bi-camera-fill"></i> Start Camera
            </button>
            <button type="button" class="btn btn-primary" id="capture-btn" style="display:none;">
              <i class="bi bi-camera2"></i> Capture Photo
            </button>
            <button type="button" class="btn btn-secondary" id="retake-btn" style="display:none;">
              <i class="bi bi-arrow-counterclockwise"></i> Retake
            </button>
          </div>
          <p class="form-help" style="margin-top:12px;">Photo is optional but recommended for identification.</p>
        </div>

        <div class="form-section">
          <div class="form-section-title"><i class="bi bi-qr-code"></i> Badge Preview</div>
          <p style="font-size:var(--text-sm);color:var(--text-muted);text-align:center;">
            A unique badge number will be auto-generated on registration.
          </p>
          <div style="text-align:center;margin-top:12px;">
            <span style="font-family:var(--font-mono);font-size:var(--text-lg);color:var(--secondary);letter-spacing:2px;">
              <?= BADGE_PREFIX ?>-<?= date('ymd') ?>-XXXXX
            </span>
          </div>
        </div>
      </div>
    ...
    <div style="display:flex;gap:12px;justify-content:flex-end;margin-top:8px;">
      <a href="<?= BASE_URL ?>pages/dashboard.php" class="btn btn-secondary"><?= __t('cancel') ?></a>
      <button type="submit" class="btn btn-primary btn-lg">
        <i class="bi bi-person-check-fill"></i> Register & Issue Badge
      </button>
    </div>
  </form>
</div>
